...
class TestConditionIdFilter:
    def test_filter_by_condition_id_returns_only_that_condition(self, openem_index):
        results = openem_index.search(
            "critical actions", top_k=10, mode="vector", condition_id="stemi"
        )
        for r in results:
    ...
    def test_filter_by_category(self, openem_index):
        results = openem_index.search(
            "treatment", top_k=10, mode="vector", category="infectious"
        )
        for r in results:
    ...
    def test_filter_by_risk_tier(self, openem_index):
        results = openem_index.search(
            "treatment", top_k=10, mode="vector", risk_tier="A"
        )
        for r in results:
...
class TestRRFMerge:
    ...
    def test_rrf_merge_boosts_overlap(self):
        ...
        ids = [r["id"] for r in merged]
        assert ids[0] == "b", (
            f"'b' should rank first (appears in both lists), got {ids}"
        )
